...
const CONN = "postgres://postgres:Akshah089#@localhost/multilingual?replication=database"
...
func main() {
	...
	defer cancel()
	psqlconn := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable replication=database", host, port, user, password, dbname)
	conn, err := pgconn.Connect(ctx, psqlconn)
	if err != nil {
		panic(err)
	}
